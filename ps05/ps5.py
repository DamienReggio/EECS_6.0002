...
def generate_models(x, y, degs):
    ...
    fit = []
    for df in degs:
        fit.append(pylab.polyfit(x, y, df))
...
def r_squared(y, estimated):
    ...
    """
    mean = sum(y)/len(y)
    dif_sq = sum((y - estimated)**2)
    dif_mean_sq = sum((y - mean)**2)
    r_sq = 1-dif_sq/dif_mean_sq
...
def evaluate_models_on_training(x, y, models):
    ...
    for model in models:
        poly = len(model)
        y_mod = numpy.polyval(model, x)

        title = 'fit for df ' + str(poly)
        xlab = 'Year'
        ylab = 'Temperature'
        if poly == 2:
            se_slope = se_over_slope(x, y, y_mod, model)
            title = 'fit for df ' + str(poly) + " se/slope: " + str(se_slope)


        pylab.plot(x, y, 'b.') # source data
        pylab.plot(x, y_mod, 'r') # model
        pylab.title(title)
        pylab.xlabel(xlab)
        pylab.ylabel(ylab)
        pylab.show()
...
'''
evaluate_models_on_training(pylab.array([1961, 1962, 1963]),
                            pylab.array([-4.4, -5.5, -6.6]),
                            [pylab.array([-1.10000000e+00, 2.15270000e+03]),
                             pylab.array([6.83828238e-14, -1.10000000e+00,
                                          2.15270000e+03])])
'''
...
def gen_cities_avg(climate, multi_cities, years):
    ...
    for year in years:
        ...
        clim_data.append(year_avg)


    clim_data = pylab.array(clim_data)
...
'''
    ...
    for year in TRAINING_INTERVAL:
        for city in ['NEW YORK']:#CITIES:
            years.append(year)
            clim_data.append(climate_data.get_daily_temp(city,1,10,year))

    years = pylab.array(years)
    clim_data = pylab.array(clim_data)
    models = generate_models(years, clim_data, [1])

    evaluate_models_on_training(years, clim_data, models)

    # Part B
    climate_data = Climate(FILENAME)

    city_avg = gen_cities_avg(climate_data, CITIES, TRAINING_INTERVAL)
